import React from 'react'

const TaskNumber = () => {
  return (
    <div className='flex items-center justify-between gap-5 mt-10'>
        <div className='bg-emerald-200 rounded w-1/2 sm:w-[48%] lg:w-[23%] text-black py-6 px-6'>
           <h1 className='font-bold text-3xl'>0</h1>
           <h1 className=' text-xl'>New Tasks</h1>
        </div>
         <div className='bg-emerald-200 rounded w-1/2 text-black py-6 px-6'>
           <h1 className='font-bold text-3xl'>0</h1>
           <h1 className=' text-xl'>Completed</h1>
        </div>
        <div className='bg-emerald-200 rounded w-full sm:w-[48%] lg:w-[23%] text-black py-6 px-6'>
           <h1 className='font-bold text-3xl'>0</h1>
           <h1 className=' text-xl'>Accepted</h1>
        </div>
         <div className='bg-emerald-200 rounded w-full sm:w-[48%] lg:w-[23%] text-black py-6 px-6'>
           <h1 className='font-bold text-3xl'>0</h1>
           <h1 className=' text-xl'>Failed</h1>
        </div>
    </div>
  )
}

export default TaskNumber
